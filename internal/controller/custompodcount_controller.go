...
// log.Info("Running Pods:%v", runningPods)

// diff := 3 - runningPods

// if diff > 0 {
// 	for i := 0; i < diff; i++ {
// 		pod := makePod(fmt.Sprintf("pod-%d", time.Now().UnixNano()), req.Namespace)
// 		if err := r.Create(ctx, pod); err != nil {
// 			log.Error(err, "Failed to create pod", "pod", pod.Name)
// 			return ctrl.Result{}, err
// 		}
// 		log.Info(fmt.Sprintf("Pod %s created", pod.Name))
// 	}
// } else if diff < 0 {
// 	log.Info(fmt.Sprintf("Deleting %d pods", -diff))
// 	for i := 0; i < -diff; i++ {
// 		pod := &corev1.Pod{
// 			ObjectMeta: metav1.ObjectMeta{
// 				Name:      fmt.Sprintf("pod-%d", time.Now().UnixNano()),
